...
const baseUrl = (process.argv[2] || config.agent?.gatewayBaseUrl || config.agent?.publicBaseUrl || '').replace(/\/$/, '');
...
await atomicJson(manifest, { id, enabled: true, base_url: baseUrl, updated_at: new Date().toISOString() });
