...
const files = ['scripts/agent/client.mjs', 'scripts/agent/cli.mjs', 'scripts/agent/mcp.mjs', 'scripts/agent/mcp-server.mjs', 'scripts/agent/verify.mjs',
  'scripts/agent/tools.mjs', 'scripts/agent/verify-graph.mjs', 'scripts/query/contract.mjs', 'scripts/server/errors.mjs', 'config/query-fields.json', 'skills/research-wiki/SKILL.md', 'skills/research-wiki/references/local-graph.md', 'docs/agent-access.md', 'docs/agent-access-migration.md', 'docs/entity-graph-agent.md'];
export async function packageAgent(destination = path.join(repo, 'outputs/researchwiki-agent')) {
  ...
  await fs.writeFile(path.join(target, 'package.json'), JSON.stringify({ name: 'researchwiki-agent-client', version: '1.2.0', private: true, type: 'module',
