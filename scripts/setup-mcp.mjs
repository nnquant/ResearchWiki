...
try{await fs.access(tokenFile);}catch{
  const result=await gb(['auth','create','investment-wiki-reader','--scopes','read']);
...
const readConfig={mcpServers:{'investment-research-wiki':{command:process.execPath,args:[path.join(repo,'scripts','mcp-stdio.mjs')]}}};
