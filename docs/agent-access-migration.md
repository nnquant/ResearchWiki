...
停止旧 Wiki 服务后运行 `npm run research:index` ，完成迁移后再启动新服务。索引指纹现在包含 profile 和字段契约，首次核对会重新构建读模型；不会调用解析器或 embedding，也不会删除保留的历史引用版本。新量化实例还需按接入说明初始化自己的 token。

## 验证

```powershell
node --test --test-concurrency=1 tests/query-contract.test.mjs tests/query-profile.test.mjs tests/agent-network.test.mjs tests/agent-skill-install.test.mjs
npm run typecheck
npm run build
```
...
详细流程参见 [分支维护](branch-maintenance.md)。
